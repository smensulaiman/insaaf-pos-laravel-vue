...
class ReportController extends BaseController
{
    ...
    public function ProfitAndLoss(request $request)
    {
        ...
        // --------expenses
        $report_total_expenses = Expense::whereBetween('date', [$start_date, $end_date])
            ->where('deleted_at', '=', null)
            ->where(function ($query) use ($warehouse_id, $array_warehouses_id) {
                if ($warehouse_id !== 0) {
                    return $query->where('warehouse_id', $warehouse_id);
        ...
        foreach ($productIds as $productId) {
            ...
            $adjustments = AdjustmentDetail::with('adjustment')
                ->where(function ($query) use ($warehouse_id, $array_warehouses_id, $end_date) {
                    if ($warehouse_id !== 0) {
                        return $query->whereHas('adjustment', function ($q) use ($warehouse_id, $end_date) {
                            $q->where('warehouse_id', $warehouse_id)
                                ->where('date', '<=', $end_date);
                        });
                    } else {
                        return $query->whereHas('adjustment', function ($q) use ($array_warehouses_id, $end_date) {
                            $q->whereIn('warehouse_id', $array_warehouses_id)
                                ->where('date', '<=', $end_date);
                        });
